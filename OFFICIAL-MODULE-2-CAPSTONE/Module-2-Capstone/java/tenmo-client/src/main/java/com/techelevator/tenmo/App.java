...
private static final String API_BASE_URL = "http://localhost:8080";
	...
	private void viewTransferHistory() {
		...
		for(Transfer transfer : theTransfers) {
			User fromUser = appServices.getUserByAccountId((long) transfer.getAccountFrom(), currentUser.getToken());
			User toUser = appServices.getUserByAccountId((long) transfer.getAccountTo(), currentUser.getToken());
	...
	private void viewPendingRequests() {
		...
		if (optionInput == 0) {
			...
			String message = appServices.approveTransfer(theTransfer, currentUser.getToken());
			System.out.println(message);
		} else {
			theTransfer.setStatusId(3);
			String message = appServices.rejectTransfer(theTransfer, currentUser.getToken());
			System.out.println(message);
		}
